...
const KNOWN_COORDS = {
  ...
  "4860 y st":{lat:38.5530,lng:-121.4480},"4860 y":{lat:38.5530,lng:-121.4480},
  "7201 arutas":{lat:38.6630,lng:-121.3760},
  "3835 freeport":{lat:38.5340,lng:-121.4930},
  "1201 k st":{lat:38.5780,lng:-121.4930},"1201 k":{lat:38.5780,lng:-121.4930},
  "1000 g st":{lat:38.5790,lng:-121.5010},"1000 g":{lat:38.5790,lng:-121.5010},
  "5735 47th":{lat:38.5250,lng:-121.4340},
  "4700 college oak":{lat:38.6290,lng:-121.3500},
  "3311 45th":{lat:38.5330,lng:-121.4390},
  "4180 green valley":{lat:38.7620,lng:-120.9760},
  "6300 fountain square":{lat:38.6940,lng:-121.2780},
  "4422 y st":{lat:38.5510,lng:-121.4470},"4422 y":{lat:38.5510,lng:-121.4470},
  "2521 stockton":{lat:38.5520,lng:-121.4530},
};
...
function cleanAddress(raw){let s=raw;
  // Truncate at known CaterTrax field names that get concatenated into address text
  const bounds=[/\bCity\b/i,/\bState\b\s+[A-Z]/i,/\bZip\s*Code/i,/\bOn\s*Site\s*Contact/i,/\bFloor\s*\/?\s*Room/i,/\bSales\s*Associate/i,/\bInvoice\s*#/i,/\bEvent\s*Title/i,/\bAlt\.\s*Contact/i,/\bCompany\s*\/?\s*Event\s*Type/i,/\bCustomer\s*Notes/i,/\bEmail\b/i,/\b(?:Mobile\s*)?Phone\b/i,/\bNeed\s*to\s*Text/i,/\bCALL\s*UPON/i];
  for(const rx of bounds){const m=s.match(rx);if(m&&m.index>3)s=s.substring(0,m.index)}
  s=s.replace(/https?:\/\/[^\s]+/gi,"");s=s.replace(/Kitchen\s*Sheet\s*Report/gi,"");s=s.replace(/\d{1,2}\/\d{1,2}\/\d{2,4}/g,"");s=s.replace(/\d{1,2}:\d{2}\s*(AM|PM)/gi,"");s=s.replace(/\d*(st|nd|rd|th)\s*Floor\b[^,]*/gi,"");s=s.replace(/\bConference\s*Room[^,]*/gi,"");s=s.replace(/\bRoom\s*#?\s*\d+[^,]*/gi,"");s=s.replace(/type=\w+[^\s]*/gi,"");s=s.replace(/\b\d{1,2}\/\d{1,2}\b/g,"");s=s.replace(/\s{2,}/g," ").trim();s=s.replace(/^[,\s]+|[,\s]+$/g,"");return s}
...
function MapView({orders,assignments,drivers,pickupRuns,driveCache}){
  ...
  useEffect(()=>{if(!mapInst.current||loading)return;const L=window.L,map=mapInst.current;layers.current.lines.forEach(l=>map.removeLayer(l));layers.current.markers.forEach(l=>map.removeLayer(l));layers.current={lines:[],markers:[]};
    const mkr=(lat,lng,color,label,pop,sz=22)=>{const icon=L.divIcon({html:`<div style="width:${sz}px;height:${sz}px;background:${color};border:2px solid rgba(0,0,0,.3);border-radius:50%;box-shadow:0 2px 8px rgba(0,0,0,.25);display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;text-shadow:0 1px 2px rgba(0,0,0,.5)">${label}</div>`,className:"",iconSize:[sz,sz],iconAnchor:[sz/2,sz/2]});const m=L.marker([lat,lng],{icon}).addTo(map);if(pop)m.bindPopup(pop,{maxWidth:280});layers.current.markers.push(m)};
  ...
  return(<div style={{flex:1,display:"flex",position:"relative"}}>
    <div style={{flex:1,position:"relative"}}>
      <div style={{position:"absolute",top:12,left:12,zIndex:1000,display:"flex",gap:8,flexWrap:"wrap"}}>
        <div style={{background:"#282e40ee",border:"1px solid #3a4258",borderRadius:8,padding:5,display:"flex",gap:2}}>
          <button onClick={()=>{setMode("delivery");setRoutePanel(null)}} style={{padding:"6px 14px",borderRadius:5,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:600,background:mode==="delivery"?"#353d52":"transparent",color:mode==="delivery"?"#f59e0b":"#8e9ab0"}}>🚐 Delivery</button>
          <button onClick={()=>{setMode("pickup");setRoutePanel(null)}} style={{padding:"6px 14px",borderRadius:5,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:600,background:mode==="pickup"?"#353d52":"transparent",color:mode==="pickup"?"#22c55e":"#8e9ab0"}}>📦 Pickup</button>
        </div>
        {mode==="delivery"&&<div style={{background:"#282e40ee",border:"1px solid #3a4258",borderRadius:8,padding:5,display:"flex",gap:2}}>
          <button onClick={()=>setSelDriver("all")} style={{padding:"5px 10px",borderRadius:4,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:10,fontWeight:600,background:selDriver==="all"?"#353d52":"transparent",color:selDriver==="all"?"#f5f7fb":"#8e9ab0"}}>All</button>
          {drivers.map(d=>{const c=(deliveryRoutes[d.id]||[]).length;if(!c)return null;return<button key={d.id} onClick={()=>{setSelDriver(d.id);setRoutePanel({type:"delivery",driverId:d.id,driverName:d.name,driverColor:d.color,stops:deliveryRoutes[d.id]})}} style={{padding:"5px 10px",borderRadius:4,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:10,fontWeight:600,background:selDriver===d.id?"#353d52":"transparent",color:selDriver===d.id?d.color:"#8e9ab0"}}><span style={{display:"inline-block",width:7,height:7,borderRadius:"50%",background:d.color,marginRight:4}}/>{d.name} ({c})</button>})}
        </div>}
      </div>
      <div style={{position:"absolute",bottom:28,left:12,zIndex:1000,background:"#282e40ee",border:"1px solid #3a4258",borderRadius:8,padding:"10px 14px",fontSize:10,color:"#8e9ab0",maxWidth:220}}>
        {mode==="delivery"?<><div style={{fontWeight:700,color:"#f5f7fb",marginBottom:6}}>Delivery Routes</div>{drivers.map(d=>{const c=(deliveryRoutes[d.id]||[]).length;if(!c)return null;return<div key={d.id} style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,cursor:"pointer"}} onClick={()=>{setSelDriver(d.id);setRoutePanel({type:"delivery",driverId:d.id,driverName:d.name,driverColor:d.color,stops:deliveryRoutes[d.id]})}}><div style={{width:18,height:3,background:d.color,borderRadius:2}}/><span>{d.name} — {c} stops</span></div>})}</>:<><div style={{fontWeight:700,color:"#f5f7fb",marginBottom:6}}>Pickup Runs</div>{pickupRuns.map((run,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,cursor:"pointer"}} onClick={()=>setRoutePanel({type:"pickup",runIndex:i,color:["#22c55e","#06b6d4","#a855f7"][i%3],windowStart:run.windowStart,windowEnd:run.windowEnd,stops:run.stops,totalDrive:run.totalDrive,totalPickupTime:run.totalPickupTime,totalTime:run.totalTime})}><div style={{width:18,height:3,background:["#22c55e","#06b6d4","#a855f7"][i%3],borderRadius:2}}/><span>Run {i+1}: {fmtShort(run.windowStart)}–{fmtShort(run.windowEnd)}</span></div>)}</>}
      </div>
      {loading&&<div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",zIndex:1000,background:"#282e40",borderRadius:8,padding:"16px 24px",color:"#8e9ab0",fontSize:12,fontWeight:600}}>Loading map...</div>}
      <div ref={mapRef} style={{width:"100%",height:"100%",background:"#e8e8e0"}}/>
    </div>
    {routePanel&&<div style={{width:340,background:"#262c3c",borderLeft:"1px solid #3a4258",overflowY:"auto",flexShrink:0}} className="fade-in">
      <div style={{padding:"14px 16px",borderBottom:"1px solid #3a4258",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,background:"#262c3c",zIndex:2}}>
        <div><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:15,color:routePanel.driverColor||routePanel.color}}>{routePanel.type==="delivery"?`${routePanel.driverName}'s Route`:`Pickup Run ${routePanel.runIndex+1}`}</div><div style={{fontSize:10,color:"#8e9ab0",marginTop:2}}>{routePanel.stops.length} stop{routePanel.stops.length>1?"s":""}{routePanel.type==="pickup"?` · ${fmtShort(routePanel.windowStart)}–${fmtShort(routePanel.windowEnd)}`:""}</div></div>
        <button onClick={()=>setRoutePanel(null)} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:5,padding:"3px 10px",color:"#8e9ab0",cursor:"pointer",fontFamily:"inherit",fontSize:11}}>✕</button>
      </div>
      <div style={{padding:"10px 16px",display:"flex",alignItems:"center",gap:10,borderBottom:"1px solid #2e3548"}}><div style={{width:28,height:28,borderRadius:"50%",background:"#f59e0b",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:14,color:"#000",flexShrink:0}}>H</div><div><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:12,color:"#f5f7fb"}}>Hannibal's HQ</div><div style={{fontSize:10,color:"#8e9ab0"}}>Depart</div></div></div>
      {routePanel.stops.map((stop,i)=>{const prev=i===0?HQ:routePanel.stops[i-1];const drv=gCD(driveCache,prev,stop);const isDel=routePanel.type==="delivery";const[bt,bc,bb]=cBadge(stop.complexity);return<div key={stop.id||i}><div style={{padding:"6px 16px 6px 30px",display:"flex",alignItems:"center",gap:8}}><div style={{width:1,height:16,background:"#444e66"}}/><span style={{fontSize:10,color:"#708098",fontStyle:"italic"}}>🚐 ~{drv}min</span></div><div style={{padding:"12px 16px",borderBottom:"1px solid #2e3548",borderLeft:`3px solid ${routePanel.driverColor||routePanel.color}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:24,height:24,borderRadius:"50%",background:routePanel.driverColor||routePanel.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"#fff",flexShrink:0}}>{i+1}</div><div><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:13,color:"#f5f7fb"}}>{stop.company}</div><div style={{fontSize:10,color:"#8e9ab0"}}>{stop.event}</div></div></div>{isDel&&<div style={{textAlign:"right"}}><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:15,color:"#f5f7fb"}}>{fmtShort(tm(stop.deliveryTime))}</div><div style={{fontSize:9,color:"#8e9ab0"}}>serve {fmtShort(tm(stop.servingTime))}</div></div>}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 12px",fontSize:11,marginTop:6}}><div><span style={{color:"#708098"}}>Guests: </span><span style={{color:"#e8ecf4"}}>{stop.guests}</span></div>{isDel&&<div><span style={{color:"#708098"}}>Setup: </span><span style={{color:"#e8ecf4"}}>{stop.setupMinutes}min</span></div>}<div style={{gridColumn:"1/-1"}}><span style={{color:"#708098"}}>📍 </span><span style={{color:"#e8ecf4",fontSize:10}}>{stop.address}</span></div>{stop.contact&&<div style={{gridColumn:"1/-1"}}><span style={{color:"#708098"}}>👤 </span><span style={{color:"#e8ecf4"}}>{stop.contact} {stop.phone}</span></div>}</div>{stop.notes&&<div style={{marginTop:6,fontSize:10,color:"#f59e0b",background:"#26200f",borderRadius:4,padding:"4px 8px"}}>⚠ {stop.notes.substring(0,120)}</div>}{!isDel&&stop.pickup&&<div style={{marginTop:6,fontSize:10,color:"#86efac"}}>📦 {stop.pickup.note}</div>}</div></div>})}
      <div style={{padding:"6px 16px 6px 30px",display:"flex",alignItems:"center",gap:8}}><div style={{width:1,height:16,background:"#444e66"}}/><span style={{fontSize:10,color:"#708098",fontStyle:"italic"}}>🚐 ~{routePanel.stops.length?gCD(driveCache,routePanel.stops[routePanel.stops.length-1],HQ):0}min return</span></div>
      <div style={{padding:"10px 16px",display:"flex",alignItems:"center",gap:10,borderTop:"1px solid #2e3548"}}><div style={{width:28,height:28,borderRadius:"50%",background:"#f59e0b",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:14,color:"#000",flexShrink:0}}>H</div><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:12,color:"#f5f7fb"}}>Return to HQ</div></div>
      <div style={{padding:"12px 16px",borderTop:"1px solid #3a4258",background:"#282e40"}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:11}}><div><span style={{color:"#708098"}}>Stops:</span> <span style={{color:"#f5f7fb",fontWeight:600}}>{routePanel.stops.length}</span></div><div><span style={{color:"#708098"}}>Guests:</span> <span style={{color:"#f5f7fb",fontWeight:600}}>{routePanel.stops.reduce((s,o)=>s+o.guests,0)}</span></div></div></div>
    </div>}
  </div>);
}
...
function UploadView({onOrdersConfirmed,initialDate}){
  ...
  if(!previewOrders){return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"#1e2230",padding:40}}>
      <div style={{textAlign:"center",maxWidth:600,width:"100%"}}>
        <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:28,color:"#f5f7fb",marginBottom:4}}>HANNIBAL'S <span style={{color:"#f59e0b"}}>DISPATCH</span></div>
        <div style={{fontSize:13,color:"#8e9ab0",marginBottom:32}}>Upload your CaterTrax Kitchen Sheet to generate the dispatch</div>
        <div onDragOver={e=>{e.preventDefault();setDragging(true)}} onDragLeave={()=>setDragging(false)} onDrop={e=>{e.preventDefault();setDragging(false);handleFile(e.dataTransfer.files[0])}} onClick={()=>fileRef.current?.click()} style={{border:`2px dashed ${dragging?"#f59e0b":"#3a4258"}`,borderRadius:12,padding:"48px 32px",cursor:"pointer",background:dragging?"#23201a":"#282e40",transition:"all .2s",marginBottom:20}}>
          <input ref={fileRef} type="file" accept=".pdf" style={{display:"none"}} onChange={e=>handleFile(e.target.files[0])}/>
          <div style={{fontSize:40,marginBottom:12}}>{parsing?"⏳":"📄"}</div>
          <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:16,color:"#e8ecf4",marginBottom:8}}>{parsing?"Parsing orders...":"Drop PDF here or click to upload"}</div>
          <div style={{fontSize:11,color:"#8e9ab0"}}>CaterTrax Kitchen Sheet Report (.pdf)</div>
        </div>
        ...
        {log.length>0&&<div style={{background:"#282e40",border:"1px solid #3a4258",borderRadius:8,padding:14,textAlign:"left",maxHeight:180,overflowY:"auto"}}>{log.map((l,i)=><div key={i} style={{fontSize:11,color:"#8e9ab0",fontFamily:"'IBM Plex Mono',monospace",lineHeight:1.8}}>{l}</div>)}</div>}
      </div>
    </div>
  )}
  ...
  // Preview & Edit
  return(
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:"#1e2230"}}>
      <div style={{background:"#262c3c",borderBottom:"1px solid #3a4258",padding:"12px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:18,color:"#f5f7fb"}}>Review & Edit <span style={{color:"#f59e0b"}}>({previewOrders.length} orders)</span></div>
          <div style={{fontSize:11,color:"#8e9ab0",marginTop:2}}>Edit details, set pickup types, then dispatch</div>
        </div>
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:11,color:"#8e9ab0"}}>Date:</span>
            <input type="date" value={dispatchDate} onChange={e=>setDispatchDate(e.target.value)} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:5,padding:"5px 10px",color:"#f5f7fb",fontFamily:"inherit",fontSize:12}}/>
          </div>
          <button onClick={()=>{setPreviewOrders(null);setLog([])}} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:6,padding:"8px 18px",color:"#8e9ab0",fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer"}}>← Re-upload</button>
        ...
        {[...previewOrders].sort((a,b)=>tm(a.deliveryTime)-tm(b.deliveryTime)).map((o,idx)=>{
          ...
          return(<div key={o.id} className="preview-card fade-in" style={{animationDelay:`${idx*30}ms`,borderLeft:`4px solid ${o.complexity==="vip"?"#fbbf24":o.complexity==="bar-event"?"#c4b5fd":"#3b82f6"}`}}>
            <div style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>setExpandedId(expanded?null:o.id)}>
              <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:16,color:"#f5f7fb",width:52}}>{fmtShort(tm(o.deliveryTime))}</div>
              <div style={{flex:1}}><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:13,color:"#e8ecf4"}}>{o.company}</div><div style={{fontSize:10,color:"#8e9ab0"}}>{o.event} · {o.address.split(",")[0]}</div></div>
              ...
              <div style={{display:"flex",gap:2}}>
                {[{t:"none",l:"No Pickup",c:"#708098",bg:"#323a50"},{t:"same-day",l:"Same-Day",c:"#86efac",bg:"#162014"},{t:"next-day",l:"Next-Day",c:"#f59e0b",bg:"#26200f"},{t:"server-cleanup",l:"Server",c:"#93c5fd",bg:"#141c2a"}].map(({t,l,c,bg})=>{
                  const active=(t==="none"&&!o.pickup)||(o.pickup?.type===t);
                  return<button key={t} onClick={e=>{e.stopPropagation();setPickupType(o.id,t)}} style={{padding:"3px 8px",borderRadius:4,border:active?`2px solid ${c}`:"1px solid #444e66",background:active?bg:"transparent",color:active?c:"#708098",fontSize:9,fontWeight:active?700:500,fontFamily:"inherit",cursor:"pointer",transition:"all .12s"}}>{l}</button>
                })}
              </div>
              <div style={{fontSize:16,color:"#708098",transition:"transform .15s",transform:expanded?"rotate(180deg)":"rotate(0)"}}>▾</div>
            ...
            {expanded&&<div style={{marginTop:12,paddingTop:12,borderTop:"1px solid #3a4258"}} className="fade-in">
              ...
              {o.pickup&&<div style={{marginTop:10,background:"#162014",border:"1px solid #244028",borderRadius:6,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:11,color:"#86efac"}}>📦 {o.pickup.note}</div><div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:10,color:"#8e9ab0"}}>Window:</span><input className="preview-input" style={{width:110,background:"#1a3020",borderColor:"#2a4030"}} value={o.pickup.window} onChange={e=>setPreviewOrders(p=>p.map(x=>x.id===o.id?{...x,pickup:{...x.pickup,window:e.target.value}}:x))} placeholder="HH:MM-HH:MM"/></div></div>}
            </div>}
          </div>)})}
      </div>
    </div>
...
function HistoryView({onLoadDate,currentDate}){
  ...
  if(loading)return<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",color:"#8e9ab0"}}>Loading history...</div>;
  return(<div style={{flex:1,overflow:"auto",padding:20}}>
    <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:18,color:"#f5f7fb",marginBottom:4}}>Dispatch History</div>
    <div style={{fontSize:12,color:"#8e9ab0",marginBottom:20}}>Click any date to load that dispatch. Current: <b style={{color:"#f59e0b"}}>{friendlyDate(currentDate)}</b></div>
    {(!history||!history.length)?<div style={{color:"#708098",fontSize:12}}>No saved dispatches yet. Upload a PDF and dispatch to save.</div>:
    <div style={{display:"grid",gap:8}}>
      {history.map(h=>{const ords=h.orders||[];const guests=ords.reduce((s,o)=>s+o.guests,0);const isCurrent=h.dispatch_date===currentDate;const isToday=h.dispatch_date===dateStr(new Date());const isPast=h.dispatch_date<dateStr(new Date());const isFuture=h.dispatch_date>dateStr(new Date());
        return<div key={h.id} onClick={()=>onLoadDate(h.dispatch_date)} style={{background:isCurrent?"#2a3450":"#282e40",border:`1px solid ${isCurrent?"#3b82f6":"#3a4258"}`,borderRadius:8,padding:"14px 18px",cursor:"pointer",transition:"all .15s",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:18,color:isToday?"#f59e0b":isFuture?"#86efac":"#e8ecf4",minWidth:90}}>{friendlyDate(h.dispatch_date)}</div>
            <div>
              <div style={{fontSize:12,color:"#e8ecf4"}}>{ords.length} orders · {guests} guests</div>
              <div style={{fontSize:10,color:"#708098",marginTop:2}}>{ords.slice(0,3).map(o=>o.company).join(", ")}{ords.length>3?` +${ords.length-3} more`:""}</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            {isPast&&!isToday&&<span style={{fontSize:9,fontWeight:700,color:"#708098",background:"#2e3548",padding:"2px 8px",borderRadius:3}}>PAST</span>}
...
export default function App(){
  ...
  const addDriver=()=>{const n=drivers.length+1;const cols=["#3b82f6","#f59e0b","#10b981","#ef4444","#8b5cf6","#ec4899","#06b6d4","#f97316"];setDrivers(p=>[...p,{id:`d${Date.now()}`,name:`Driver ${n}`,color:cols[(n-1)%cols.length],available:"07:00",end:"16:00"}])};
  const removeDriver=(did)=>{if(drivers.length<=1)return;setDrivers(p=>p.filter(d=>d.id!==did));const u={...asgn};Object.keys(u).forEach(k=>{if(u[k]===did)delete u[k]});setAsgn(u);rebuild(u);if(selDrv===did)setSelDrv(null);if(editDrv===did)setEditDrv(null)};
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <div className="hdr">
        <div style={{display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
          ...
          {saveStatus&&<div style={{fontSize:10,padding:"3px 8px",borderRadius:4,fontWeight:600,background:saveStatus==="saved"?"#162014":saveStatus==="saving"?"#282e40":"#201414",color:saveStatus==="saved"?"#86efac":saveStatus==="saving"?"#8e9ab0":"#fca5a5"}}>{saveStatus==="saved"?"✓ Saved":saveStatus==="saving"?"Saving...":"Save error"}</div>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          {/* Date switcher */}
          <div style={{display:"flex",alignItems:"center",gap:4}}>
            <button onClick={()=>{const d=new Date(currentDate+"T12:00:00");d.setDate(d.getDate()-1);loadDate(dateStr(d))}} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:4,padding:"4px 8px",color:"#8e9ab0",cursor:"pointer",fontFamily:"inherit",fontSize:12}}>◀</button>
            <input type="date" value={currentDate} onChange={e=>loadDate(e.target.value)} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:5,padding:"4px 10px",color:"#f5f7fb",fontFamily:"inherit",fontSize:12}}/>
            <button onClick={()=>{const d=new Date(currentDate+"T12:00:00");d.setDate(d.getDate()+1);loadDate(dateStr(d))}} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:4,padding:"4px 8px",color:"#8e9ab0",cursor:"pointer",fontFamily:"inherit",fontSize:12}}>▶</button>
          </div>
          <button onClick={()=>setPhase("upload")} style={{background:"#353d52",border:"1px solid #4e5a74",borderRadius:5,padding:"5px 14px",color:"#f59e0b",fontFamily:"inherit",fontSize:11,fontWeight:600,cursor:"pointer"}}>📄 Upload</button>
          <div className="tabs">
      ...
      <div className="grid">
        ...
        <div className="side">
          <div className="sec">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div className="stitle" style={{marginBottom:0}}>Drivers</div><button onClick={addDriver} style={{background:"#353d52",border:"1px solid #4e5a74",borderRadius:4,padding:"2px 8px",color:"#8e9ab0",cursor:"pointer",fontFamily:"inherit",fontSize:10,fontWeight:600}}>+ Add</button></div>
            {drivers.map(d=>{const cnt=Object.values(asgn).filter(a=>a===d.id).length;const g=stats.dl[d.id]||0;const isE=editDrv===d.id,isD=dropT===d.id;const isSel=selDrv===d.id;
              const dOrds=[...orders].filter(o=>asgn[o.id]===d.id).sort((a,b)=>tm(a.deliveryTime)-tm(b.deliveryTime));
              return<div key={d.id}><div className={`dcard ${isE?"editing":""} ${isD?"drop-target":""}`} style={{borderLeft:`3px solid ${d.color}`,marginBottom:isSel?0:6,borderBottomLeftRadius:isSel?0:7,borderBottomRightRadius:isSel?0:7}} onClick={()=>{if(isE)return;setSelDrv(isSel?null:d.id)}} onDragOver={e=>dOver(e,d.id)} onDragLeave={dLeave} onDrop={e=>dDrop(e,d.id)}>
                <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:5}}>
                  <div className="ddot" style={{background:d.color}}/>
                  {isE?<input className="dinput" value={d.name} onChange={e=>{e.stopPropagation();setDrivers(p=>p.map(x=>x.id===d.id?{...x,name:e.target.value}:x))}} onClick={e=>e.stopPropagation()} autoFocus/>:<span style={{fontWeight:600,color:"#e8ecf4",fontSize:12,fontFamily:"'DM Sans',sans-serif",flex:1}}>{d.name}</span>}
                  <button onClick={e=>{e.stopPropagation();setEditDrv(isE?null:d.id)}} style={{background:"none",border:"none",color:isE?"#3b82f6":"#708098",cursor:"pointer",fontSize:12,padding:2}} title="Edit">✏️</button>
                  {isE&&drivers.length>1&&<button onClick={e=>{e.stopPropagation();if(confirm(`Remove ${d.name}?`))removeDriver(d.id)}} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:11,padding:2,opacity:0.7}} title="Remove">🗑</button>}
                </div>
                {isE?<div style={{display:"flex",flexDirection:"column",gap:4,fontSize:10,color:"#8e9ab0"}}><div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{width:36}}>Start:</span><input className="dinput" type="time" value={d.available} onChange={e=>{e.stopPropagation();setDrivers(p=>p.map(x=>x.id===d.id?{...x,available:e.target.value}:x))}} onClick={e=>e.stopPropagation()} style={{width:100}}/></div><div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{width:36}}>End:</span><input className="dinput" type="time" value={d.end} onChange={e=>{e.stopPropagation();setDrivers(p=>p.map(x=>x.id===d.id?{...x,end:e.target.value}:x))}} onClick={e=>e.stopPropagation()} style={{width:100}}/></div></div>:<div style={{fontSize:10,color:"#8e9ab0"}}>{fmtShort(tm(d.available))} – {fmtShort(tm(d.end))}</div>}
                <div style={{fontSize:10,color:"#8e9ab0",marginTop:3}}>{cnt} deliver{cnt!==1?"ies":"y"} · {g} guests</div>
              </div>
              {/* Expanded delivery list */}
              {isSel&&!isE&&<div style={{background:"#2a3040",border:"1px solid #3a4258",borderTop:"none",borderBottomLeftRadius:7,borderBottomRightRadius:7,marginBottom:6,overflow:"hidden"}} className="fade-in">
                {!dOrds.length?<div style={{padding:"10px 12px",fontSize:10,color:"#708098",fontStyle:"italic"}}>No deliveries assigned</div>:
                dOrds.map((o,i)=>{const prev=i===0?HQ:dOrds[i-1];const drv=gCD(dc,prev,o);const[bt,bc,bb]=cBadge(o.complexity);
                  return<div key={o.id} style={{padding:"8px 12px",borderBottom:i<dOrds.length-1?"1px solid #333c50":"none",cursor:"pointer"}} onClick={e=>{e.stopPropagation();setSelOrder(o.id);setView("timeline")}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:9,fontWeight:700,color:d.color,background:"rgba(255,255,255,0.08)",borderRadius:3,padding:"1px 5px",minWidth:18,textAlign:"center"}}>{i+1}</span>
                        <span style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:13,color:"#f5f7fb"}}>{fmtShort(tm(o.deliveryTime))}</span>
                      </div>
                    ...
                    <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:11,color:"#e8ecf4"}}>{o.company}</div>
                    <div style={{fontSize:9,color:"#8e9ab0",marginTop:1}}>{o.address.split(",")[0]} {o.room?`· ${o.room}`:""}</div>
                    <div style={{display:"flex",gap:10,fontSize:9,color:"#708098",marginTop:3}}>
                      <span>🚐 {drv}min</span><span>⏱ {o.setupMinutes}min setup</span><span>serve {fmtShort(tm(o.servingTime))}</span>
                    </div>
                    {o.pickup&&<div style={{fontSize:9,color:"#86efac",marginTop:3}}>📦 {o.pickup.note}</div>}
                    {o.notes&&<div style={{fontSize:9,color:"#f59e0b",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>⚠ {o.notes}</div>}
                  </div>})}
                {dOrds.length>0&&<div style={{padding:"8px 12px",background:"#252a38",display:"flex",justifyContent:"space-between",fontSize:10}}>
                  <span style={{color:"#708098"}}>Total: {dOrds.reduce((s,o)=>s+o.guests,0)} guests</span>
                  <span style={{color:"#708098"}}>Est. route: {dOrds.reduce((s,o,i)=>s+gCD(dc,i===0?HQ:dOrds[i-1],o),0)+gCD(dc,dOrds[dOrds.length-1],HQ)}min drive</span>
                </div>}
              </div>}
              </div>})}
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          {view==="timeline"&&<><div className="tl-wrap"><div style={{minWidth:1100}}>
            ...
            {drivers.map(d=>{const blocks=sched[d.id]||[];const tot=TL_END-TL_START;return<div key={d.id} className={`tl-row ${dropT===d.id?"drop-over":""}`} onDragOver={e=>dOver(e,d.id)} onDragLeave={dLeave} onDrop={e=>dDrop(e,d.id)}>
              <div className="tl-lbl"><div className="ddot" style={{background:d.color}}/><span style={{fontSize:11,fontWeight:600,color:"#e8ecf4",fontFamily:"'DM Sans',sans-serif"}}>{d.name}</span></div>
              <div className="tl-track">{HR.map((_,i)=><div key={i} style={{position:"absolute",left:`${(i/HR.length)*100}%`,top:0,bottom:0,width:1,background:"#2e3548"}}/>)}
          ...
          {selOrder&&(()=>{const o=orders.find(x=>x.id===selOrder);if(!o)return null;const drv=gCD(dc,HQ,o),mi=hvMi(HQ,o).toFixed(1);
            return<div className="detail fade-in" key={selOrder}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}><div><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:17,color:"#f5f7fb"}}>{o.company}</div><div style={{fontSize:12,color:"#8e9ab0"}}>{o.event} · #{o.id}</div></div><button onClick={()=>setSelOrder(null)} style={{background:"#323a50",border:"1px solid #444e66",borderRadius:5,padding:"3px 10px",color:"#8e9ab0",cursor:"pointer",fontFamily:"inherit",fontSize:11}}>✕</button></div>
          ...
          {view==="orders"&&<div style={{flex:1,overflow:"auto",padding:20}}><div style={{display:"grid",gap:10}}>
            {[...orders].sort((a,b)=>tm(a.deliveryTime)-tm(b.deliveryTime)).map((o,idx)=>{const[bt,bc,bb]=cBadge(o.complexity);
              return<div key={o.id} className="fade-in" style={{animationDelay:`${idx*25}ms`,background:"#282e40",border:"1px solid #3a4258",borderRadius:9,padding:16,borderLeft:`4px solid ${dcol(asgn[o.id])}`}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><div><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:16,color:"#f5f7fb"}}>{o.company}</div><div style={{fontSize:11,color:"#8e9ab0",marginTop:2}}>{o.event}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:20,fontWeight:800,color:"#f5f7fb",fontFamily:"'DM Sans',sans-serif"}}>{fmtShort(tm(o.deliveryTime))}</div></div></div>
          ...
          {view==="pickups"&&<div style={{flex:1,overflow:"auto",padding:20}}>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:18,color:"#f5f7fb",marginBottom:16}}>Pickup Route Optimizer</div>
            {pkRuns.map((run,ri)=><div key={ri} className="pk-run fade-in"><div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:15,color:"#86efac"}}>Run {ri+1}: {fmtShort(run.windowStart)} – {fmtShort(run.windowEnd)}</div><div style={{fontSize:14,color:"#86efac",fontWeight:700}}>{run.totalTime}min</div></div>
              {run.stops.map((st,si)=>{const drv=gCD(dc,si===0?HQ:run.stops[si-1],st);return<div key={st.id}><div style={{padding:"4px 0 4px 10px",fontSize:9,color:"#8e9ab0",fontStyle:"italic"}}>🚐 {drv}min →</div><div className="pk-stop"><div className="pk-num">{si+1}</div><div style={{flex:1}}><div style={{fontSize:12,fontWeight:700,color:"#e8ecf4",fontFamily:"'DM Sans',sans-serif"}}>{st.company}</div><div style={{fontSize:10,color:"#8e9ab0"}}>{st.address}</div><div style={{fontSize:10,color:"#86efac",marginTop:4}}>{st.pickup.note}</div></div><div style={{fontSize:10,color:"#8e9ab0"}}>{st.guests}p</div></div></div>})}
          ...
          {view==="settings"&&<div style={{flex:1,overflow:"auto",padding:20,maxWidth:680}}>
            <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:800,fontSize:18,color:"#f5f7fb",marginBottom:16}}>Settings</div>

            {/* Supabase */}
            <div className="api-bar">
              <div style={{fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:13,color:"#e8ecf4",marginBottom:8}}>Supabase Connection</div>
              <div style={{fontSize:11,color:"#8e9ab0",marginBottom:10}}>Connect to Supabase to save dispatches across sessions. Create a table first:</div>
              <div style={{background:"#1e2230",borderRadius:6,padding:12,fontSize:11,fontFamily:"'IBM Plex Mono',monospace",color:"#8e9ab0",lineHeight:1.8,marginBottom:12,whiteSpace:"pre-wrap",overflowX:"auto"}}>{`CREATE TABLE dispatch_days (

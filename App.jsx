...
function cleanAddress(raw) {
  ...
  // Remove "Kitchen Sheet Report" and similar headers
  s = s.replace(/Kitchen\s*Sheet\s*Report/gi, "");
  // Remove date/time patterns: 2/26/26, 3:34 PM, 2/26/2026
  s = s.replace(/\d{1,2}\/\d{1,2}\/\d{2,4}/g, "");
  s = s.replace(/\d{1,2}:\d{2}\s*(AM|PM)/gi, "");
  // Remove floor/room/conference references (captured separately in room field)
  s = s.replace(/\d*(st|nd|rd|th)\s*Floor\b[^,]*/gi, "");
  s = s.replace(/\b(Suite|Ste|Bldg|Building)\s+\S+/gi, (match) => match); // keep suite numbers
  s = s.replace(/\bConference\s*Room[^,]*/gi, "");
  s = s.replace(/\bRoom\s*#?\s*\d+[^,]*/gi, "");
  // Remove "type=kitchen..." query fragments
  s = s.replace(/type=\w+[^\s]*/gi, "");
  // Remove stray fragments like "2/9" that aren't part of address
  s = s.replace(/\b\d{1,2}\/\d{1,2}\b/g, "");
  // Collapse whitespace
  s = s.replace(/\s{2,}/g, " ").trim();
  // Remove leading/trailing commas and whitespace
  s = s.replace(/^[,\s]+|[,\s]+$/g, "");
  // If we end up with just city/state or nothing meaningful, return as-is
  return s;
}

function extractRoom(raw) {
  // Pull floor/room/conference info from the raw address text
  const parts = [];
  const floorM = raw.match(/(\d*(?:st|nd|rd|th)\s*Floor\b[^,]*)/i);
  if (floorM) parts.push(floorM[1].trim());
  const confM = raw.match(/(Conference\s*Room[^,]*)/i);
  if (confM) parts.push(confM[1].trim());
  const roomM = raw.match(/(Room\s*#?\s*\d+[^,]*)/i);
  if (roomM && !parts.some(p => p.includes(roomM[1]))) parts.push(roomM[1].trim());
  return parts.join(", ");
}
...
export default function App() {
  ...
  const [apiSt, setApiSt] = useState("none");
  ...
  const rebuild = useCallback((a, cache, o) => {
    const ords = o || orders, c = cache || dc;
    setSched(buildSched(ords, a, drivers, c));
  ...
  const fetchMaps = useCallback(async () => {
    if (!apiKey) { setApiSt("missing"); return; }
    setMapsL(true); setApiSt("loading");
    try { const c = await buildDriveCache(apiKey, [HQ, ...orders]); setDc(c); const a = autoAssign(orders, drivers, c); setAsgn(a); rebuild(a, c, orders); setApiSt("ok"); }
    catch (e) { setApiSt(e.message?.includes("Failed to fetch") ? "cors" : "error"); }
    setMapsL(false);
  ...
  const dDrop = (e, did) => { e.preventDefault(); setDropT(null); if (!drag) return; const u = { ...asgn, [drag.orderId]: did }; setAsgn(u); rebuild(u, dc, orders); setDrag(null); };
  const dEnd = (e) => { if (e.target?.style) e.target.style.opacity = "1"; setDrag(null); setDropT(null); };
  const reassign = (oid, did) => { const u = { ...asgn, [oid]: did }; setAsgn(u); rebuild(u, dc, orders); };

  const stats = useMemo(() => {
    const g = orders.reduce((s, o) => s + o.guests, 0);
    const e = orders.length ? orders.reduce((m, o) => Math.min(m, tm(o.deliveryTime)), Infinity) : 0;
    const l = orders.length ? orders.reduce((m, o) => Math.max(m, tm(o.deliveryTime)), 0) : 0;
    const dl = {}; drivers.forEach(d => { dl[d.id] = 0 }); Object.entries(asgn).forEach(([oid, did]) => { const o = orders.find(x => x.id === oid); if (o && dl[did] != null) dl[did] += o.guests; });
    return { n: orders.length, g, e: e ? fmt(e) : "--", l: l ? fmt(l) : "--", dl };
  }, [orders, asgn, drivers]);
